export interface DataCenter {
  ...
  lat: number;
  lng: number;
  energyConsumption: number; // MWh per day
  waterConsumption: number; // L per day
  noiseLevel: number; // dB from 100ft
...
// Parse CSV data
const csvData = `Locality,Name,Owner_Applicant,Street_Address,Acres,Building_Sq_Ft,MW_reported,MW estimated,MWh per day,L per day,dB from 100ft,Construction_Type,Build_Status,Lat,Long,Year Operational
...
function parseCSV(csv: string): DataCenter[] {
  ...
  return lines.slice(1).map((line, index) => {
    ...
    return {
      ...
      locality: values[0] || 'Unknown',
      lat: parseFloat(values[13]) || 0,
      lng: parseFloat(values[14]) || 0,
      energyConsumption: parseFloat(values[8]) || 0,
      waterConsumption: parseFloat(values[9]) || 0,
      noiseLevel: parseFloat(values[10]) || 0,
      buildingArea: parseFloat(removeCommas(values[5])) || 0,
      yearOperational: parseInt(values[15]) || 2020,
    };
  });
}

export const dataCenters: DataCenter[] = parseCSV(csvData);
